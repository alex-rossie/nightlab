...
Steps:
1. Re-read every PR merged in the last 7 days. For each experiment PR, check that the replication criterion was written before the result, that the verdict matches the criterion, that the ledger entry's commit is a commit of that PR (`gh pr view N --json commits`), and that the PR is merged. Open an issue for any mismatch.
